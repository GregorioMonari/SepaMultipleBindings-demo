...
class HttpClient {
    constructor(hostname){
        ...
        this.log.info("New Http Client created! Host: "+this.hostname)
    }
    //============POST AND AUTH POST===================//
    auth_post(url,data,content_type){
        ...
        return new Promise(resolve=>{
            ...
            req.addEventListener('loadstart', ()=>{
                this.log.info("keycloak-client: POST "+url)
                this.log.info("post data: "+data)
            });
            req.addEventListener('error', ()=>{this.log.info(req.responseText)});
            ...
            req.onreadystatechange = function() {// Call a function when the state changes.
                ...
                if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
                    ...
                    this.log.info("keycloak-client: auth post completed")
                    resolve(req.responseText);
                }else{
                    if(this.readyState === XMLHttpRequest.DONE && this.status === 201){
                        this.log.info("keycloak-client: auth post completed, status 201")
                        resolve(req.responseText);  
                    }
                }
            }
